...
open OUT16, '>', 'All_Sig_H1';
open OUT17, '>', 'All_Sig_H2';
open OUT18, '>', 'All_Sig_H1Sites';
open OUT19, '>', 'All_Sig_H2Sites'; 
...
print OUT16 "GENE\tNTAX\tSEQLEN\tCLADE_BACKGROUND\tCLADE_FOREGROUND\tEFFECTOR\n";
print OUT17 "GENE\tNTAX\tSEQLEN\tAllCQF_BACKGROUND\tAllCQF_FOREGROUND\tEFFECTOR\n";
print OUT18 "GENE\tNTAX\tSEQLEN\tCLADE_PROPORTION_POSITIVE_SITES\tEFFECTOR\n";
print OUT19 "GENE\tNTAX\tSEQLEN\tAllCQF_PROPORTION_POSITIVE_SITES\tEFFECTOR\n";
$H1count = 0;
$H1ecount = 0;
$H2count = 0;
$H2ecount = 0;
...
while (<FH1>)
{
	if (/(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+\S+\s+\S+\s+\S+\s+\S+\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+\S+\s+\S+\s+(\S+)\s+(\S+)\s+\S+\s+\S+\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)/)
	{
		$Gene = $1;
		$SeqNum = $2;
		$SeqLen = $3;
...
#---------------------------------------------------------------------------------------#		
		if ($LRT1 <= 1.9985E-02 && $LRT2 > 2.1097E-02 && $LRT3 > 1.7678E-02 && $LRT4 > 1.5298E-02)
		{
			...
			print OUT16 "$Gene\t$SeqNum\t$SeqLen\t$H1_Background\t$H1_Foreground\t$Effector\n";
		}
		if ($LRT1 > 1.9985E-02 && $LRT2 <= 2.1097E-02 && $LRT3 > 1.7678E-02 && $LRT4 > 1.5298E-02)
		{
			...
			print OUT17 "$Gene\t$SeqNum\t$SeqLen\t$H2_Background\t$H2_Foreground\t$Effector\n";
		...
		if ($LRT1 > 1.9985E-02 && $LRT2 > 2.1097E-02 && $LRT3 <= 1.7678E-02 && $LRT4 > 1.5298E-02)
		{
			...
			print OUT18 "$Gene\t$SeqNum\t$SeqLen\t$H1_ProportionSites\t$Effector\n";
		}
		if ($LRT1 > 1.9985E-02 && $LRT2 > 2.1097E-02 && $LRT3 > 1.7678E-02 && $LRT4 <= 1.5298E-02)
		{
			...
			print OUT19 "$Gene\t$SeqNum\t$SeqLen\t$H2_ProportionSites\t$Effector\n";
		}
		if ($LRT1 > 1.9985E-02 && $LRT2 > 2.1097E-02 && $LRT3 <= 1.7678E-02 && $LRT4 <= 1.5298E-02)
		{
...
$SigEffectorTests = ($H1ecount + $H2ecount + (2 * $H1H2ecount) + $H1eSitescount + $H2eSitescount + (2 * $H1eSitesH2eSitescount) + (2 * $H1H1Sitescounte) + (2 * $H1H2Sitescounte) + (2 * $H2H1Sitescounte) + (2 * $H2H2Sitescounte) + (3 * $H1H2H1Sitescounte) + (3 * $H1H2H2Sitescounte) + (3 * $H1H1SitesH2Sitescounte) + (3 * $H2H1SitesH2Sitescounte) + (4 * $H1H2H1SitesH2Sitescounte));
...
print OUT99 "Total Number of Tests: $TotalTests\nTotal Number of Significant Tests at FDR=0.05: $TotalSigThings\n";
print OUT99 "Significant H1: $SigH1\nSignificant H2: $SigH2\nSignificant H1 Sites: $SigH1Sites\nSignificant H2 Sites: $SigH2Sites\n\n";
print OUT99 "Total Number of Effectors: $TotalEffectors\n";
print OUT99 "Total Number of Tests performed with effectors: $TotalEffectorTests\n";
print OUT99 "Significant Tests with effectors: $SigEffectorTests\n\n";
