...
close FH2000;
#----------------------------------------------
system "ls *.codonalign.paml.phy > PhyFiles";
open FH2001, '<', 'PhyFiles';
%AlignNums = ();
%AlignLengths = ();
while (<FH2001>)
{
	if (/((\S+)\.codonalign\.paml\.phy)/)
	{
		$File = $1;
		$Gene = $2;
		open FH2002, "$File";
...
print OUT1 "Gene\tNumSeqs\tAlignLength\tEffector\tH0_lnL\tH1_lnL\tH2_lnL\tH0_Global_dN/dS\tH1_Background_dN/dS\tH1_Foreground_dN/dS\tH2_Background_dN/dS\tH2_Foreground_dN/dS\tH1_ConstrainSites_lnL\tH1_PositiveSites_lnL\tH1_PositiveClass1\tH1_PositiveClass2\tH2_ConstrainSites_lnL\tH2_PositiveSites_lnL\tH2_PositiveClass1\tH2_PositiveClass2\tLRT1(H1_vs_H0)\tLRT2(H2_vs_H0)\tLRT3(H1_PositiveSites_vs_H1_ConstrainSites)\tLRT4(H2_PositiveSites_vs_H2_ConstrainSites)\n";
foreach $GENE (keys %GeneHash)
{
	if (exists $OneRateLik{$GENE})
	{
		print OUT1 "$GENE\t$AlignNums{$GENE}\t$AlignLengths{$GENE}\t";
