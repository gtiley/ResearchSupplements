...
print OUT1 "Gene\tH0_lnL\tH1_lnL\tH2_lnL\tH0_Global_dN/dS\tH1_Background_dN/dS\tH1_Foreground_dN/dS\tH2_Background_dN/dS\tH2_Foreground_dN/dS\tH1_ConstrainSites_lnL\tH1_PositiveSites_lnL\tH1_PositiveClass1\tH1_PositiveClass2\tH2_ConstrainSites_lnL\tH2_PositiveSites_lnL\tH2_PositiveClass1\tH2_PositiveClass2\tLRT1(H1_vs_H0)\tLRT2(H2_vs_H0)\tLRT3(H1_PositiveSites_vs_H1_ConstrainSites)\tLRT4(H2_PositiveSites_vs_H2_ConstrainSites)\n";
foreach $GENE (keys %GeneHash)
{
	if (exists $OneRateLik{$GENE})
	{
		print OUT1 "$GENE\t$OneRateLik{$GENE}\t";
	...
	#		$ChiProb1 = Statistics::Distributions::chiprob (1, $LRT1);
	#		$ChiProb2 = Statistics::Distributions::chiprob (1, $LRT2);
			print OUT1 "$LRT1\t$LRT2\t$LRT3\t$LRT4\n";
		}
	...
	#		print "$TwoRateDNDS{$GENE}{Foreground}\n";
	#		$LRT1 = (2 * ($BurstLik{$GENE} - $OneRateLik{$GENE}));	
			$LRT2 = (2 * ($TwoRateLik{$GENE} - $OneRateLik{$GENE}));
			$LRT4 = (2 * ($AltTwoLik{$GENE} - $NullTwoLik{$GENE}));
	#		$ChiProb1 = Statistics::Distributions::chiprob (1, $LRT1);
	#		$ChiProb = Statistics::Distributions::chiprob (1, $LRT);
			print OUT1 "NA\t$LRT2\tNA\t$LRT4\n";
		}	
	}
}
close OUT1;
